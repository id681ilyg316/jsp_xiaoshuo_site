...
<body>
	...
	<div id="content" class="center">
	   <p class="p_head">为您找到小说</p>
	   <div>
	     <%
	     	String key = request.getParameter("keyword");
	     	if (key == null)
	     		return;
	     	String encodeKey=new String(key);
	     	PostDao pd = new PostDao();
	     	UserDao ud=new UserDao();
	     	List<Post> ps = pd.getPostsByTtile("%" + encodeKey + "%");
	     	if (ps.size() == 0) {
	     		out.print("<p style='text-align:center;line-height:24px;'>没有找到相应的小说</p>");
	     	} else {
	     		for (Post p : ps) {
	     		    User u=ud.getUser(p.getUser_id());
	     			out.print("<div class='today_p_wrap'>");
	     			out.print("<img src='/xiaoshuo_site/images/" + u.getUser_icon() + "' width='36px' height='36px'>");
	     ...
	     <p class="p_head" style="margin-top:20px">为您找到用户</p>
	       <%
	     	...
	     	if (uu.size() == 0) {
	     		...
	     		for (User ur : uu) {
	     			out.print("<div class='today_p_wrap'>");
	     			out.print("<img src='/xiaoshuo_site/images/" + ur.getUser_icon() + "' width='36px' height='36px'>");
	     			out.print("<p class='today_p u_p'>");
	     			out.print("&nbsp;<a href='/xiaoshuo_site/jsp/userdetail.jsp?uid=" +ur.getUser_id()+ "'>");
